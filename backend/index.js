...
<div id="app" class="hide">
  ...
  <div id="v-rels" class="hide">
    ...
    <div class="card" style="border-color:rgba(34,211,238,.3)"><h2>🧪 تست قبل از انتشار</h2>
      <p style="font-size:.73rem;opacity:.75;line-height:2.1">۱) روی «⬇️ دانلود تست» در کادر پیش‌نویس بزن تا فایل زیپ اکستنشن دانلود شود.<br>۲) زیپ را در یک پوشهٔ خالی استخراج (Extract) کن.<br>۳) در کروم برو به <b dir="ltr">chrome://extensions</b> — کلید <b>Developer mode</b> (بالا سمت راست) را روشن کن، دکمهٔ <b>Load unpacked</b> را بزن و پوشهٔ استخراج‌شده را انتخاب کن.<br>۴) اکستنشن را کامل تست کن؛ اگر همه‌چیز درست بود، «✅ تایید و انتشار» را بزن.</p>
    </div>
  ...
  <div id="v-ann" class="hide">
    <div class="card"><h2>📢 اطلاعیه به همه کاربران اکستنشن</h2>
      ...
      <span class="lbl">🔗 لینک دکمهٔ دانلود (اختیاری — کنار اطلاعیه نمایش داده می‌شود)</span>
      <input id="annlink" dir="ltr" placeholder="https://…">
...
function dlAsset(id,name){msg('در حال آماده‌سازی فایل تست…');fetch('/admin/gh/asset?id='+id+'&name='+encodeURIComponent(name),{headers:{'x-admin-key':K()}}).then(function(r){if(!r.ok)throw new Error(r.status);return r.blob();}).then(function(b){var a=document.createElement('a');a.href=URL.createObjectURL(b);a.download=name;document.body.appendChild(a);a.click();a.remove();msg('فایل تست دانلود شد 🧪 — استخراج کن و در کروم Load unpacked بزن',true);}).catch(function(e){msg('خطا در دانلود: '+e.message);});}
...
function clearAnn(){api('/admin/settings',{settings:{announce_text:null,announce_link:null,announce_start:null,announce_end:null,announce_at:null}}).then(function(){msg('اطلاعیه حذف شد',true);loadAnn();}).catch(function(e){msg(e.message);});}
...
function loadAnn(){api('/admin/settings').then(function(s){document.getElementById('anntxt').value=s.settings.announce_text||'';document.getElementById('annlvl').value=s.settings.announce_level||'info';var toLocal=function(ms){return ms?new Date(parseInt(ms,10)-new Date().getTimezoneOffset()*60000).toISOString().slice(0,16):'';};var at=document.getElementById('annat');if(at)at.value=toLocal(s.settings.announce_start||s.settings.announce_at);var en=document.getElementById('annend');if(en)en.value=toLocal(s.settings.announce_end);var al=document.getElementById('annlink');if(al)al.value=s.settings.announce_link||'';});}
function saveAnn(){var at=document.getElementById('annat').value;var ms=at?new Date(at).getTime():0;var enEl=document.getElementById('annend');var mE=enEl&&enEl.value?new Date(enEl.value).getTime():0;var alEl=document.getElementById('annlink');api('/admin/settings',{settings:{announce_text:document.getElementById('anntxt').value.trim()||null,announce_level:document.getElementById('annlvl').value,announce_at:ms>Date.now()?ms:null,announce_start:ms?ms:null,announce_end:mE?mE:null,announce_link:alEl&&alEl.value.trim()?alEl.value.trim():null}}).then(function(){msg(ms>Date.now()?'اطلاعیه زمان‌بندی شد ⏰':'اطلاعیه منتشر شد 📢',true);}).catch(function(e){msg(e.message);});}
function loadFlags(){api('/admin/settings').then(function(s){var f=(s.settings&&s.settings.flags)||{};var defs=[['prices','💱 ویجت قیمت ارز و طلا'],['weather','🌤️ ویجت آب‌وهوا'],['quotes','💫 جملهٔ روز'],['chat','💬 چت و دوستان'],['wallpapers','🖼️ پنل والپیپرها'],['fal','📜 فال حافظ'],['pray','🕌 اوقات شرعی و قبله'],['blocker','🚫 مسدودساز سایت']];var h='';defs.forEach(function(d){h+='<label style="display:flex;gap:10px;align-items:center;margin:9px 0;font-size:.8rem;cursor:pointer"><input type="checkbox" style="width:auto" data-flag="'+d[0]+'"'+(f[d[0]]===false?'':' checked')+'><span>'+d[1]+'</span></label>';});document.getElementById('flags-list').innerHTML=h;}).catch(function(e){msg(e.message);});}
...
async function handle(req, env) {
  ...
  try {
    ...
    if (p === '/admin/gh/releases' && req.method === 'GET') {
      ...
      return json({
        releases: g.data.map(r => ({
          ...
          assets: (r.assets || []).map(a => ({ id: a.id, name: a.name, size: a.size, url: a.browser_download_url, downloads: a.download_count || 0 }))
        }))
      });
    }
    ...
    if (p === '/admin/gh/asset' && req.method === 'GET') {
      if (!hasKey) return err('forbidden', 403);
      const aid = String(url.searchParams.get('id') || '');
      if (!/^\d+$/.test(aid)) return err('bad_asset_id');
      const r = await fetch('https://api.github.com/repos/Taboraex/tabora/releases/assets/' + aid, {
        headers: {
          'Authorization': 'Bearer ' + (env.GITHUB_TOKEN || ''),
      ...
      if (r.status !== 200) return err('asset_fetch_' + r.status, 502);
      const name = String(url.searchParams.get('name') || ('asset-' + aid + '.zip')).replace(/[^a-zA-Z0-9._-]/g, '_');
      await log(db, 'test', 'test download: ' + name);
    ...
    if (p === '/api/announce') {
      ...
      return json({ text: String(s.announce_text || ''), level: String(s.announce_level || 'info'), link: String(s.announce_link || '') });
    }
